...
az storage account create --name $env:TERRAFORM_STORAGE_ACCOUNT --resource-group $env:RG_NAME --location $env:LOCATION --sku Standard_LRS
Write-Output "FINISHED: Creating Storage Account."

# Storage Container
Write-Output "STARTED: Creating Storage Container..."
az storage container create --name $env:TF_CONTAINER_NAME --account-name $env:TERRAFORM_STORAGE_ACCOUNT
